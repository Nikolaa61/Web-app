    ...
    <div v-if="vest">
      ...
      <form @submit.prevent="komentarisi">
        <div class="form-group">
          ...
          <input v-model="autor" type="text" class="form-control" id="username" aria-describedby="usernameHelp"
...
export default {
  ...
  methods: {
    ...mapActions(['izmeni_kategoriju', 'ucitaj_vesti']),
    formatDate(date) {
  ...
  mounted() {
    for (let i = 0; i < this.vesti.length; i++) {
      if (this.vesti[i].id === parseInt(this.$route.params.id)) {
        this.vest = this.vesti[i];
      }
    }
    fetch(`http://localhost:8081/api/komentari/${this.$route.params.id}`,  { method: 'get' }).then((response) => {
      if (!response.ok)
        throw response;
      return response.json();
    }).then((jsonData) => {
